...
namespace SAP_Core.DAL
{
    public class DepositDAL : Connection, IDisposable
    {
        public ListDeposit GetCobranzaC (string imei, string fecIni, string fecFin)
        {
            ...
            string strSQL = string.Format("CALL {0}.APP_DEPOSITS ('{1}','{2}','{3}')", DataSource.bd(), imei, fecIni, fecFin);
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                    ...
                    deposito.U_VIS_CollectionSalesPerson = reader["U_VIS_CollectionSalesPerson"].ToString();
                    deposito.U_VIS_BankValidation = reader["U_VIS_BankValidation"].ToString();
                    deposito.U_VIS_Photo = reader["U_VIS_Photo"].ToString();
            ...
            catch (Exception ex)
            {
                ...
                strSQL = string.Format("CALL {0}.ins_msg_proc('{1}','{2}','{3}')", DataSource.bd(), "APP Sales Force GET", "Error", "Despacho_GetCobranzaC - " + ex.Message +" "+imei+" "+fecIni+" "+fecFin);
                HanaCommand command = new HanaCommand(strSQL, connection);
        ...
        public ListDeposit bankValidation(string imei)
        {
            HanaDataReader reader;
            HanaConnection connection = GetConnection();
            ListDeposit depositos = new ListDeposit();
            List<DepositBO2> listaDepositos = new List<DepositBO2>();
            DepositBO2 deposito = new DepositBO2();
            string strSQL = string.Format("CALL {0}.APP_DEPOSITS_BANK_VALIDATION ('{1}')", DataSource.bd(), imei);
            ...
            catch (Exception ex)
            {
                ...
                strSQL = string.Format("CALL {0}.ins_msg_proc('{1}','{2}','{3}')", DataSource.bd(), "APP Sales Force GET", "Error", "Despacho_GetCobranzaC - " + ex.Message + " " + imei);
                HanaCommand command = new HanaCommand(strSQL, connection);
        ...
        public DepositBO2 GetCobranza(string imei, string banco, string deposito)
        {
            ...
            string strSQL  = string.Format("CALL {0}.APP_DEPOSITS_BANK ('{1}','{2}','{3}')", DataSource.bd(), imei, banco, deposito);
            ...
            catch (Exception ex)
            {
                ...
                strSQL = string.Format("CALL {0}.ins_msg_proc('{1}','{2}','{3}')", DataSource.bd(), "APP Sales Force GET", "Error", "Despacho_GetCobranza - " + ex.Message+" "+imei
                    +" " +banco
                    +" " +deposit);
                HanaCommand command = new HanaCommand(strSQL, connection);
        ...
        public DepositResponse GetValidDeposit(string bank, string deposit, decimal ammount, string date, string slpCode)
        {
            ...
            string strSQL = string.Format("CALL {0}.APP_VALID_DEPOSIT ('{1}','{2}',{3},'{4}','{5}')", DataSource.bd(), bank, deposit, ammount, date, slpCode);
            ...
            catch (Exception ex)
            {
                ...
                strSQL = string.Format("CALL {0}.ins_msg_proc('{1}','{2}','{3}')", DataSource.bd(), "APP Sales Force GET", "Error", "Despacho_GetValidDeposit - " + ex.Message
                    +" "+bank
                    +" " +deposit
                    +" " +ammount
                    +" " +date
