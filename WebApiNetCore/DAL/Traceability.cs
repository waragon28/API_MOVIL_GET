...
namespace SAP_Core.DAL
{
    public class TraceabilityDAL : Connection, IDisposable
    {
        public ListTraceability GetTraceability(string imei, string fecha)
        {
            ...
            string strSQL  = string.Format("CALL {0}.APP_SALES_ORDER_TRACEABILITY ('{1}','{2}')", DataSource.bd(), imei, fecha);
        ...
        public ListSalesRoute GetSalesRoute(string imei, string fecha)
        {
            HanaDataReader reader;
            HanaConnection connection = GetConnection();
            List<SalesRouteBO> ListTraceability = new List<SalesRouteBO>();
            ListSalesRoute traceabilties = new ListSalesRoute();
            SalesRouteBO traceability = new SalesRouteBO();
            string strSQL = string.Format("CALL {0}.APP_SALES_ROUTE ('{1}','{2}')", DataSource.bd(), imei, fecha);
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        traceability = new SalesRouteBO();
                        traceability.CardCode = reader["CardCode"].ToString();
                        traceability.Address = reader["Address"].ToString();
                        traceability.Chk_Visit = reader["Chk_Visit"].ToString();
                        traceability.Chk_Pedido = reader["Chk_Pedido"].ToString();
                        traceability.Chk_Cobranza = reader["Chk_Cobranza"].ToString();
                        traceability.Chk_Ruta = reader["Chk_Ruta"].ToString();
                        traceability.FechaRuta = reader["FechaRuta"].ToString();
                        ListTraceability.Add(traceability);
                    }
                }
                traceabilties.SalesRoute = ListTraceability;
                connection.Close();
