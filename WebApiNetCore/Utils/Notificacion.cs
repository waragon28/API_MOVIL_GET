...
namespace WebApiNetCore.Utils
{
    ...
    public class Notificacion: INotificationService
    {
        ...
        {
            ResponseData response = await serviceLayer.Request( "/b1s/v1/"+ endPoint + "("+ docEntry + ")?$select=NumAtCard,BusinessPartner&$expand=BusinessPartner($select=Phone1, CardName)", Method.GET);
